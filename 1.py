...
# ─────────────────────────────────────────
def get_conn():
    return sqlite3.connect('valuation_data.db')
...
def delete_entry(eid):
    conn = get_conn()
    conn.cursor().execute("DELETE FROM evaluations WHERE id=?", (eid,))
...
with tab_input:
    ...
    if edit_id:
        ...
        st.info(f"📝 수정 모드 — ID #{edit_id}  |  [새로 작성으로 전환하려면 아래 버튼 클릭]")
        if st.button("➕ 새로 작성"):
    ...
    purpose = r0c3.selectbox("감정목적",
        ["담보", "매매참고", "소송", "상속/증여", "기타"],
        index=["담보","매매참고","소송","상속/증여","기타"].index(load_data['purpose'])
              if load_data.get('purpose') in ["담보","매매참고","소송","상속/증여","기타"] else 0)
    ...
    l_area  = lc1.number_input("적용면적 (㎡)",    value=float(load_data.get('land_area', 0.0)),  step=0.1)
    l_price = lc2.number_input("토지 단가 (원/㎡)", value=int(load_data.get('land_price', 0)),    step=1000)
    ...
    b_addr = st.text_input("건물 비고(명칭 등)", value=load_data.get('b_addr', ''))
    bc1, bc2, bc3, bc4 = st.columns(4)
    b_re_cost = bc1.number_input("재조달원가 (원/㎡)", value=int(load_data.get('b_re_cost', 0)))
    b_area    = bc2.number_input("건물 면적 (㎡)",     value=float(load_data.get('b_area', 0.0)))
    b_life    = bc3.number_input("내용연수 (년)",       value=int(load_data.get('b_total_life', 40)))
    b_passed  = bc4.number_input("경과연수 (년)",       value=int(load_data.get('b_passed', 0)))
    ...
    u_addr  = st.text_input("구분건물 비고(단지명/호수)", value=load_data.get('u_addr', ''))
    uc1, uc2, uc3 = st.columns(3)
    u_area  = uc1.number_input("전유면적 (㎡)",          value=float(load_data.get('u_area', 0.0)))
    u_price = uc2.number_input("구분건물 단가 (원/㎡)",   value=int(load_data.get('u_price', 0)))
    ...
    with save_col:
        save_label = "💾 수정 저장" if edit_id else "💾 신규 저장"
...
with tab_list:
    ...
    conn = get_conn()
    sido_list = pd.read_sql_query("SELECT DISTINCT si_do FROM evaluations WHERE si_do != ''", conn)['si_do'].tolist()
    conn.close()
    sido_filter = fc2.selectbox("시도 필터", ["전체"] + sido_list)
    date_from = fc3.date_input("시작 기준일", value=None)
    date_to   = fc4.date_input("종료 기준일", value=None)
    ...
    if df.empty:
        ...
        st.download_button("⬇️ 엑셀 내보내기", data=excel_bytes,
        ...
        for _, row in df.iterrows():
            ...
            col_a.markdown(f"**#{row['id']}** {addr} {purpose_tag}", unsafe_allow_html=True)
            col_a.caption(f"기준: {row['ref_date']}  |  등록: {row['reg_date']}  |  의뢰인: {row.get('client_name','')}")
            col_b.markdown(f'<div style="text-align:right;font-family:IBM Plex Mono,monospace;font-weight:600;color:#c0392b">₩ {int(row["grand_total"]):,}</div>', unsafe_allow_html=True)
            if col_c.button("✏️ 수정", key=f"edit_{row['id']}"):
                st.session_state.edit_id = int(row['id'])
                st.rerun()
            if col_d.button("🗑️ 삭제", key=f"del_{row['id']}"):
...
with tab_report:
    ...
    if all_df.empty:
        ...
        all_df['label'] = all_df.apply(lambda x: f"[{x['id']}] {x['si_do']} {x['si_gun_gu']} {x['eup_myeon_dong']} {x['main_bun']}-{x['sub_bun']}", axis=1)
        ...
        full_addr = f"{r['si_do']} {r['si_gun_gu']} {r['eup_myeon_dong']} {r['bun_type']} {r['main_bun']}-{r['sub_bun']}"
